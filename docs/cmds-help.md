...
* where - откуда считыать пресеты: local - из локальной директории, s3 - на Amazon S3.
...
* Если where === s3 - запрашивает содержимое корзины с Amazon S3 и выдаем вывод.
* Если указан параметр short, то выводится просто список.
...
* Генерится рандомный ключ блокировки (6 HEX цифр).
