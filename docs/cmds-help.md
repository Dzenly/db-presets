...
* Все файлы (и стянутые с Amazon S3 и те, которые были раньше в папке с архивами) расшифровываются.
* Раз-xz-уются.
...
или последний в папке с архивами (если `only` не задали), оказывается актуальным.
...
* **Важно**: Если у вас были несохраненные изменения в каком-то пресете - они исчезнут, т.к. пресет заново
восстановится из архива, и на него накатятся миграции и локализация.
* При совместной работе время от времени важно делать `get`, чтобы подкачивать изменения других
или чтобы просто накатить новые миграции и локализации на существующие пресеты.
