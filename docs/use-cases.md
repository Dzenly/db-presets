...
`db-p reset-lock name=my-preset who=my-name`

### Шаги / результат

* Снимается твой лок, если ты передумал редактировать пресет.

### Замечания.

* Можно снять только свой лок.
...
* Проверка, что на файле стоит лок и он того юзера, который who. Если лока нет - ругнуться и выйти.
* `pg_dumpall -h 127.0.0.1 -U postgres --clean > <tmp директория внутри самого модуля>`
* xz файла (кол-во потоков = кол-ву процессоров, уровень сжатия - максимальный)
